...
#define FD_COPY(set,orgset) memcpy(&set,&orgset,sizeof(fd_set))
		...
		#ifdef _WIN32
			//由于win32fd是数组，以sets为主set，增加其他的set
			if (ret>0) {
				FD_COPY(set,rset);
	...
	#endif
		int max;
		fd_set orgset;//每次select时从orgset复制，检查fd信号时则使用set，这时就算调用add，del等也不会影响计数
		fd_set rorgset;
		fd_set sorgset;
		fd_set eorgset;
		fd_set set;
		...
		*/
		...
		virtual void free_socket(SOCKET fd)
		{
			...
			if (obj) {
				...
				sendpool->del(fd);
				//fixme 有个问题就是resend里有某个buf，然后刚加到sendqueue，发送失败freesocket时，直接就把sendqueue的buf释放了，resend再发就会有问题
						...
						if (selectpool->iserr(i))
							...
							if (selectpool->isrecv(i)){
								...
								if (ret<=0) {
									...
									//buf指针已考虑这些情况：包括onreceive返回0，数据暂无法处理，没有eventcb，数据都已处理
